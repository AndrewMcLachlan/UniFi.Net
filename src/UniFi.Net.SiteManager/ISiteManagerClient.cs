...
public interface ISiteManagerClient
{
    ...
    /// <summary>
    /// Gets ISP metrics for the specified interval, optionally filtered by duration.
    /// </summary>
    /// <param name="type">The metric interval (5-minute or 1-hour).</param>
    /// <param name="duration">The time range of metrics to retrieve, starting from the current time.</param>
    /// <param name="cancellationToken">A <see cref="CancellationToken"/> to observe while waiting for the task to complete.</param>
    /// <returns>ISP metrics data.</returns>
